---
title: "Compute per-barcode deer ACE2 binding affinity"
author: "Tyler Starr"
date: "05/12/2023"
output:
  github_document:
...
We successfully generated *K*<sub>D</sub> estimates for `r sum(!is.na(dt[library=="pool1",Kd_deer]))` of our pool1 barcodes (`r round(sum(!is.na(dt[library=="pool1",Kd_deer]))/nrow(dt[library=="pool1",])*100,digits=2)`%), and `r sum(!is.na(dt[library=="pool2",Kd_deer]))` of our pool2 barcodes (`r round(sum(!is.na(dt[library=="pool2",Kd_deer]))/nrow(dt[library=="pool2",])*100,digits=2)`%). 
...
We have generated binding measurements for `r round(nrow(dt[!is.na(log10Ka_deer)])/nrow(dt)*100,digits=2)`% of the barcodes in our libraries.
