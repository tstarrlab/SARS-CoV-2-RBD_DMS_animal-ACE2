# Deep mutational scans of SARS-CoV-2 RBDs for animal ACE2s

Analysis of deep mutational scanning of barcoded codon variants of SARS-CoV-2 (Wuhan-Hu-1, Alpha, and Omicron BA.2 variants) for binding to ACE2 from white-tailed deer, Syrian hamster, domestic cat, and little brown bat.
...
The `snakemake` pipeline will automatically output a YAML file detailing all packages and versions loaded into the computing environment with each execution of the pipeline. This file [environment.yml](./environment.yml) (and any version history) should retain a record of the exact software versions that were used over the course of pipeline execution.
