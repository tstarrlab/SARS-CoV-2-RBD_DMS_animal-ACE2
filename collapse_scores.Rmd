...
---
This notebook reads in the per-barcode Kd measurements from the `compute_xACE2_Kd` scripts. It synthesizes these per-ACE2 results into the final 'mean' phenotypes of each mutant and integrates the prior huACE2 measurements.
...
We have duplicates for each measurement. Let's look at correlations! First, pooling all three targets in one plot

```{r plot_correlations_all-bg, echo=T, fig.width=8, fig.height=8, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,2))
...
```{r plot_correlations_WH1, echo=T, fig.width=8, fig.height=8, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,2))
...
```{r plot_correlations_WH1_n_bc, echo=T, fig.width=8, fig.height=8, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,2))
...
```{r plot_correlations_Eta, echo=T, fig.width=8, fig.height=8, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,2))
...
```{r plot_correlations_E484K_n_bc, echo=T, fig.width=8, fig.height=8, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,2))
...
Correlations for Omicron BA.2 background. For hamster and deer, seeing lots of variants look like they have higher ACE2 affinity in rep2 bu tlow in rep1. Taking a brief look through the mutations, it seems like these mutants have more barcodes in rep1 and just 1 or 2 barcodes in rep2, suggesting the rep2 measurement is the 'errant' one. Not sure why this would happen, from a brief look it doesn't look like an ACE2 mixup pattern (but worth looking at this more carefully). Might need to look at some of these with the interactive curve plots? Why weren't there more bcs with these mutations making it through in replicate 2? Some to look back at include mutations F12Q, I88C, V181F (many more that can be looked through, just wnated to note a few to circle back on). I have the per-rep binding and n_bc metrics in the final data output file below so I can look these mutations back up when going back to the titraiton fits to see waht might be happening.

```{r plot_correlations_BA2, echo=T, fig.width=8, fig.height=8, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,2))
...
```{r plot_correlations_BA2_n_bc, echo=T, fig.width=8, fig.height=8, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,2))
...
Collapse down to mean from both replicates, and total n barcodes between the two/three replicates. Also record the number of the replicates the variant was quantified within. Note, we are currently keeping a value even if it's determined from a single bc fit in a single pool. Later on, we may want to require some combination of minimum number of bcs within or between libraries for retention. Another option (or a combo of both) is to not take the mean of the per-replicate measurements, but rather the mean across all barcodes (so if one value is the mean of 5 bcs, and one is just 1 bc, the overall mean will be weighted toward the 5 as it's not collapse to replciate, then mean of replicate)
...
```{r integrate_prior_huACE2, echo=T, fig.width=8, fig.height=4, fig.align="center", dpi=300,dev="png"}
...
Look at correlations in mutation effects between each background and huACE2, for bind phenotype

```{r plot_correlations_by_bg_bind, echo=T, fig.width=8, fig.height=12, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(3,2))
...
```
Make heatmaps showing raw affinity and delta-affinity of muts relative to wildtype for each of the ACE2s. First, log10Ka for deer ACE2

```{r heatmap_DMS_log10Ka_deer, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka_deer, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_log10Ka_hamster, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka_hamster, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_log10Ka_bat, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka_bat, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_log10Ka_cat, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka_cat, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_log10Ka_human_monomer, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka_human_monomer, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_log10Ka_human_dimer, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka_human_dimer, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
Then, the other grouping for heatmaps is, for each background, faceted heatmaps for each ACE2

Wuhan-Hu-1, absolute affinities

```{r heatmap_DMS_log10Ka_Wuhan-Hu-1, fig.width=25,fig.height=20,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka_Wuhan-Hu-1, fig.width=25,fig.height=20,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_log10Ka_Eta, fig.width=25,fig.height=17,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka_Eta, fig.width=25,fig.height=17,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_log10Ka_Omicron_BA2, fig.width=25,fig.height=17,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka_Omicron_BA2, fig.width=25,fig.height=17,fig.align="center", dpi=500,dev="png",echo=T}
